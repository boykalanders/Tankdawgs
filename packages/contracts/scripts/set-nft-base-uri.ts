...
/**
 * Set the TankDawgsNFT metadata base URI (owner-only).
 *
 ...
 *   BASE_URI="https://tankdawgs-web.vercel.app/nft/" \
 *   pnpm --filter @tankdawgs/contracts exec hardhat run scripts/set-nft-base-uri.ts --network sepolia
 *
...
async function main() {
  ...
  if (!nftAddress) throw new Error("Set NFT_ADDRESS to the TankDawgsNFT address");
  ...
  const nft = await ethers.getContractAt("TankDawgsNFT", nftAddress);
