 ...
 * existing TankDawgs escrow — the clean, retroactive path.
 *
 * Steps:
 *   1. Deploy a fresh TankDawgsNFT with the given BASE_URI (read-time metadata).
 *   2. Upgrade the TankDawgs proxy so it gains `setDDawgsNFT`.
 ...
 *
 *   BASE_URI="https://backend.chessdawgs.io/v1/nft/tankdawgs/" \
 *   HOLDERS="0xabc...,0xdef..." \
 *   pnpm --filter @tankdawgs/contracts exec hardhat run scripts/redeploy-nft.ts --network sepolia
 *
 * Afterwards set NEXT_PUBLIC_TANKDAWGS_NFT_ADDRESS (web) to the printed address
...
async function main() {
  ...
  const nft = await (await ethers.getContractFactory("TankDawgsNFT")).deploy(baseURI);
  await nft.waitForDeployment();
  const nftAddress = await nft.getAddress();
  console.log(`New TankDawgsNFT → ${nftAddress}  (baseURI ${baseURI})`);

  // 2. Upgrade the escrow so it has setDDawgsNFT, then 3. re-point the gate.
  const TankDawgs = await ethers.getContractFactory("TankDawgs");
  const pool = await upgrades.upgradeProxy(proxyAddress, TankDawgs);
  ...
  console.log(`  NEXT_PUBLIC_TANKDAWGS_NFT_ADDRESS=${nftAddress}`);
}
