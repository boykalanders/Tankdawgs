 ...
 *   • TankDawgsNFT    — the real mintable membership pass (the play gate)
 *   • MockDDawgsNFT   — stands in for the ChessDawgs NFT (grandfather exception)
 *   • TankDawgs proxy — owned by the deployer (= backend relayer)
 *
...
async function main() {
  ...
  // images from token #1 — e.g. "https://backend.example.io/v1/nft/tankdawgs/".
  const nft = await (await ethers.getContractFactory("TankDawgsNFT")).deploy(
  ...
  // at a dedicated signer instead if the server uses a separate key.
