...
import { TANK_DAWGS_ABI, type Address } from "@tankdawgs/shared";
...
/**
 * Mirrors TankDawgs contract events into the lobby for the browse list.
 *
...
export function startChainListener(
  ...
  async function scan(from: number, to: number): Promise<void> {
    ...
    for (const ev of await contract.queryFilter(contract.filters.GameJoined(), from, to)) {
      const a = argsOf<{ gameId: string; player: string; seat: bigint }>(ev);
      // Seat 0 is the creator (already counted by upsertCreated); later seats fill.
      if (Number(a.seat) > 0) lobby.markJoined(a.gameId);
    }
