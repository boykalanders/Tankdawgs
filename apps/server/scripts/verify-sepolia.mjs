...
async function main() {
  ...
  console.log(`\nVerifying TankDawgs on chainId ${net.chainId} (${dep.network})`);
  ...
  check("DDawgsNFT == TankDawgsNFT", eq(await pool.DDawgsNFT(), dep.poolDawgsNFT));
  ...
  check("NFT is Tank Dawgs / PDAWG", (await nft.name()) === "Tank Dawgs" && (await nft.symbol()) === "PDAWG");
