...
import { ERC721_ABI, TANK_DAWGS_NFT_ABI, type Address } from "@tankdawgs/shared";
import { CHESS_NFT_ADDRESS, TANKDAWGS_NFT_ADDRESS } from "./env";
...
async function poolPassImage(client: Client, owner: Address): Promise<string | null> {
  if (!TANKDAWGS_NFT_ADDRESS) return null;
...
async function resolveNftImage(client: Client, owner: Address): Promise<string | null> {
  // Prefer the TankDawgs membership pass; fall back to a grandfathered
